/**
 * pii.js — local PII detection + crackability scoring.
 *
 * Ported verbatim from frontend/src/lib/piiEngine.js. Same regex, same
 * weights, same scoring formula — the CLI and the web app must agree on
 * what counts as PII.
 *
 * Pure JS, no dependencies, no network.
 */
'use strict';
...
/**
 * Render the segments returned by redactText into a flat string with `[TYPE]`
 * placeholders in place of the detected PII. Convenience helper for CLI use.
 */
function renderRedacted(segments) {
  return segments
    .map(s => (s.kind === 'redact' ? `[${s.type}]` : s.text))
    .join('');
}
 ...
 */
function scorePassword(pw, profile = {}) {
...
function generateWordlist(profile, limit = 40) {
