...
const ResultPage = () => {
  ...
  const qualityPct =
    wordlist.length > 0
      ? Math.round(
          wordlist
            .slice(0, Math.min(20, wordlist.length))
            .reduce((sum, w) => sum + (getScore(w) ?? 50), 0) / Math.min(20, wordlist.length),
        )
      : 0;
  ...
  const handleDownloadTxt = () => {
    ...
    a.href = url;
    a.download = `wordlist_${historyId || 'generated'}.txt`;
  ...
  if (!wordlist || wordlist.length === 0)
    return (
      <DesignAppShell>
        ...
        >
          <AlertTriangle
            style={{ width: 48, height: 48, color: 'var(--accent-500)', opacity: 0.6 }}
          />
          <div
            style={{
              fontSize: 16,
  ...
  return (
    <DesignAppShell>
      ...
      {usedFallback && (
        ...
        >
          <AlertTriangle
            style={{ width: 18, height: 18, color: '#f59e0b', flexShrink: 0, marginTop: 1 }}
          />
          <div style={{ fontSize: 12, color: 'var(--fg-2)', lineHeight: 1.5 }}>
            <span style={{ fontWeight: 700, color: '#f59e0b', fontFamily: 'var(--font-mono)' }}>
              ALGORITHMIC MODE
            </span>{' '}
            — the AI engine was unavailable, so this wordlist was produced by the local permutation
            engine. Results are still valid but less profile-tailored. Configure
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              <button
                onClick={historyId ? handleDownloadPdf : undefined}
                disabled={!historyId}
                style={{
                  height: 42,
                  borderRadius: 6,
                  ...
                  fontWeight: 500,
                  backgroundColor: 'var(--ink-1)',
                  color: historyId ? 'var(--fg-0)' : 'var(--fg-3)',
                  border: '1px solid var(--ink-4)',
                  cursor: historyId ? 'pointer' : 'not-allowed',
                  opacity: historyId ? 1 : 0.5,
                  transition: 'all 0.2s',
                }}
                onMouseEnter={(e) => {
                  if (historyId) e.currentTarget.style.background = 'var(--ink-3)';
                }}
                onMouseLeave={(e) => {
                  if (historyId) e.currentTarget.style.background = 'var(--ink-1)';
                }}
              >
