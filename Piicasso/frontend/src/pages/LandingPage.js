...
function HeroDemo() {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', gap: 6 }}>
          {['#ff5f56', '#ffbd2e', '#27c93f'].map((c) => (
            <span
              key={c}
              style={{ width: 10, height: 10, borderRadius: 5, background: c, opacity: 0.55 }}
            />
          ))}
        </div>
        <div
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
          <span
            className="dot pulse"
            style={{ background: scanning ? 'var(--accent-500)' : 'var(--good)' }}
          />
          <span
            style={{
              fontFamily: 'var(--font-mono)',
      ...
      <div
        style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', minHeight: 280 }}
        className="hero-demo-grid"
      >
        ...
        >
          {segments.map((s, i) =>
            s.kind === 'redact' ? (
              <RedactToken key={i} label={s.label}>
                {s.text}
              </RedactToken>
            ) : (
...
function Hero() {
  ...
  return (
    <section
      style={{ position: 'relative', overflow: 'hidden', borderBottom: '1px solid var(--ink-4)' }}
    >
      <div
        className="grid-bg"
        style={{
  ...
  return (
    <Section style={{ padding: '80px var(--gutter) 56px' }}>
      ...
      >
        ...
        >
          <a
            href="https://www.npmjs.com/package/piicasso"
            target="_blank"
            rel="noreferrer"
          ...
          <a
            href="https://pypi.org/project/piicasso/"
            target="_blank"
            rel="noreferrer"
            style={{ color: 'var(--accent-500)', textDecoration: 'none' }}
          >
...
function ModePanel({ tone, eyebrow, title, desc, stat, statLabel, cta, onCta, accentVar }) {
  return (
    ...
    >
      ...
      />
      <div className="eyebrow" style={{ color: `var(${accentVar})`, marginBottom: 16 }}>
        ● {eyebrow}
      </div>
      <h3 className="h-display" style={{ fontSize: 36, marginBottom: 16 }}>
        {title}
      </h3>
...
function SplitModes() {
  ...
  return (
    <Section>
      ...
      <h2
        className="h-display"
        style={{ fontSize: 'clamp(36px, 5vw, 56px)', maxWidth: 720, marginBottom: 40 }}
      >
        Same password.
        <br />
...
function FeatureGrid() {
  const features = [
    {
      eb: '01',
      t: 'Profile-aware mutation',
      d: 'Combine names, dates, places, pets, employers, ZIPs into millions of plausible candidates ranked by likelihood.',
    },
    {
      eb: '02',
      t: 'Threat intel visualization',
      d: 'A 3D Mission Control surface that lights up the relationships between leaked records and weak credentials.',
    },
    {
      eb: '03',
      t: 'Policy enforcement at scale',
      d: "Drop our SDK into auth flows. Reject passwords that contain the user's own PII before they ever ship.",
    },
    {
      eb: '04',
      t: 'Air-gapped & on-prem',
      d: 'Run the engine entirely inside your VPC. Nothing leaves. Nothing phones home.',
    },
    {
      eb: '05',
      t: 'Streaming detection',
      d: 'Sub-millisecond classification across 14 entity types. PCRE-clean. Unicode-safe.',
    },
    {
      eb: '06',
      t: 'Open audit trail',
      d: 'Every detection, every redaction, every rejected password — cryptographically signed and exportable.',
    },
  ];
  return (
    <Section>
      ...
      <h2
        className="h-display"
        style={{ fontSize: 'clamp(36px, 5vw, 56px)', maxWidth: 720, marginBottom: 56 }}
      >
      ...
      >
        {features.map((f, i) => (
          ...
          >
            ...
            >{`// ${f.eb}`}</div>
            <div
              style={{ fontSize: 18, fontWeight: 500, marginBottom: 10, letterSpacing: '-0.01em' }}
            >
              {f.t}
            </div>
...
function LiveDemo() {
  ...
  const cracked =
    wordlist.includes(pw) || wordlist.some((w) => pw.toLowerCase().startsWith(w.toLowerCase()));
  ...
  return (
    <Section>
      ...
      <h2
        className="h-display"
        style={{ fontSize: 'clamp(36px, 5vw, 56px)', maxWidth: 800, marginBottom: 16 }}
      >
              ...
              <div style={{ display: 'grid', gap: 6 }}>
                {result.reasons.length === 0 && (
                  <div style={{ color: 'var(--fg-3)', fontSize: 13 }}>
                    No PII or pattern matches. Nice.
                  </div>
                )}
...
if (result.crackable) {
  ...
  return (
    <Section>
      <div
        style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: 64, alignItems: 'center' }}
        className="api-snippet-grid"
      >
        <div>
          <div className="eyebrow" style={{ marginBottom: 16 }}>
            Drop-in policy
          ...
          <h2
            className="h-display"
            style={{ fontSize: 'clamp(32px, 4vw, 48px)', marginBottom: 16 }}
          >
            Six lines of code.
            <br />
            One fewer breach.
...
function Testimonials() {
  const items = [
    {
      quote:
        'We replaced three different tools — leak monitoring, password policy, and a homegrown wordlist generator — with PIIcasso in an afternoon.',
      who: 'Priya Raman',
      role: 'Head of Security, Helix',
    },
    {
      quote:
        'The mutation engine found credentials that twelve months of internal red-teaming missed. Embarrassing. Useful.',
      who: 'Marcus Webb',
      role: 'Principal Red Team, Northwind',
    },
    {
      quote:
        "I'm not technical and I finally understand why my password is bad. The score speaks for itself.",
      who: 'Sam K.',
      role: 'Marketing director, on User mode',
    },
  ];
  return (
    <Section>
      ...
      <h2
        className="h-display"
        style={{ fontSize: 'clamp(32px, 4vw, 48px)', marginBottom: 48, maxWidth: 700 }}
      >
  ...
  return (
    <Section style={{ padding: '120px var(--gutter)' }}>
      ...
      >
        ...
        />
        <div className="eyebrow" style={{ marginBottom: 20, position: 'relative' }}>
          ● The work
        </div>
        <h2
          className="h-display"
