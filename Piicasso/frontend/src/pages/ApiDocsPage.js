...
const CodeBlock = ({ code, id, copiedId, onCopy, label = 'example' }) => (
  <div className="terminal-block overflow-hidden">
    <div
      className="terminal-dots flex items-center justify-between border-b border-white/[0.04] px-4 py-2"
      style={{ ...S.mono, fontSize: 11, color: C.dim }}
    >
      <span>{label}</span>
      <button
    ...
    <pre
      ...
      style={{ ...S.mono, color: C.muted, lineHeight: 1.7 }}
    >
      <code>{code}</code>
    </pre>
  </div>
...
const MethodBadge = ({ method }) => {
  ...
  return (
    <span
      className={`rounded border px-2.5 py-0.5 text-[10px] font-bold tracking-wider ${bgColor} ${textColor} ${borderColor}`}
      style={S.mono}
    >
      {method}
    </span>
...
const endpointGroups = [
  ...
  },
  {
    ...
    description:
      'Submit PII data to generate targeted password wordlists using AI pattern matching combined with RockYou corpus.',
    endpoints: [
      {
        method: 'POST',
        path: '/api/submit/',
        desc: 'Submit PII data (name, DOB, pet names, etc.) to generate a custom wordlist',
      },
      {
        method: 'GET',
        path: '/api/cached/<cache_key>/',
        desc: 'Retrieve a recently generated wordlist from Redis cache',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        method: 'GET',
        path: '/api/history/',
        desc: 'Get paginated generation history (supports page, page_size query params)',
      },
      {
        method: 'DELETE',
        path: '/api/history/<id>/',
        desc: 'Delete a specific history entry (owner or admin only)',
      },
      ...
      {
        method: 'GET',
        path: '/api/report/pdf/<id>/',
        desc: 'Download PDF report for a specific generation',
      },
      {
        method: 'POST',
        path: '/api/download-token/',
        desc: 'Generate a short-lived signed download token (60s expiry)',
      },
      {
        method: 'GET',
        path: '/api/file/<file_type>/<id>/',
        desc: 'Download file using signed token (no JWT in URL)',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        ...
        desc: 'Analyze password strength with PII context and breach check',
      },
      { method: 'GET', path: '/api/password/history/', desc: 'Get password analysis history' },
      {
        method: 'GET',
        path: '/api/password/preferences/',
        desc: 'Get user preferences (default mode, last mode)',
      },
      {
        method: 'PUT',
        path: '/api/password/preferences/',
        desc: 'Update user preferences (user/security mode)',
      },
      {
        method: 'POST',
        path: '/api/password/breach-check/',
        desc: 'Check password against HIBP database (k-anonymity)',
      },
      {
        method: 'GET',
        path: '/api/password/activity/',
        desc: 'Get user activity feed (analyses, audit logs)',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        method: 'GET',
        path: '/api/profile/',
        desc: 'Get detailed profile (includes team info, unread messages, stats)',
      },
      { method: 'PUT', path: '/api/profile/', desc: 'Update profile (name, email, password)' },
      {
        method: 'GET',
        path: '/api/stats/',
        desc: 'Get user statistics (operations, passwords generated)',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        method: 'GET',
        path: '/api/teams/',
        desc: 'Get team info (members, role, invite code, activity feed)',
      },
      {
        method: 'POST',
        path: '/api/teams/create/',
        desc: 'Create a new team (creator becomes LEADER)',
      },
      { method: 'POST', path: '/api/teams/join/', desc: 'Join an existing team via invite code' },
      {
        method: 'POST',
        path: '/api/teams/leave/',
        desc: 'Leave current team (LEADER transfers or dissolves)',
      },
      {
        method: 'GET',
        path: '/api/teams/chat/',
        desc: 'Get team chat messages (supports ?after=message_id)',
      },
      {
        method: 'POST',
        path: '/api/teams/chat/',
        desc: 'Send a message to team chat (max 2000 chars)',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        method: 'GET',
        path: '/api/operations/messages/',
        desc: 'List messages (user→admin thread or admin view all)',
      },
      {
        method: 'POST',
        path: '/api/operations/messages/',
        desc: 'Send a message (user to admin, or admin to any user)',
      },
      {
        method: 'POST',
        path: '/api/operations/messages/<id>/mark_read/',
        desc: 'Mark a specific message as read',
      },
      {
        method: 'GET',
        path: '/api/operations/notifications/',
      ...
      },
      {
        method: 'DELETE',
        path: '/api/operations/settings/',
        desc: 'Delete a system setting (admin only)',
      },
      {
        method: 'POST',
        path: '/api/operations/breach-search/',
        desc: 'Search data breaches (HIBP API + internal history)',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        method: 'GET',
        path: '/api/analytics/globe-data/',
        desc: 'Get geo-located user activity for the interactive globe (?since=ISO timestamp)',
      },
      {
        method: 'POST',
        path: '/api/analytics/beacon/',
        desc: 'HELP beacon signal (frontend keepalive, requires auth)',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        ...
        desc: 'Get all users, logs, activities, total generations (superuser only)',
      },
      {
        method: 'POST',
        path: '/api/super-admin/',
        desc: 'Admin actions: block/unblock user, change password',
      },
      {
        method: 'DELETE',
        path: '/api/super-admin/',
        desc: 'Delete a user and all their data (superuser only)',
      },
      {
        method: 'GET',
        path: '/api/admin/users/',
        desc: 'List all non-superuser accounts for admin messaging',
      },
      {
        method: 'GET',
        path: '/api/messages/',
        desc: 'Admin: list user conversations; User: get admin conversation',
      },
      { method: 'POST', path: '/api/messages/', desc: 'Send message (admin↔user)' },
      {
        method: 'GET',
        path: '/api/system/logs/',
        desc: 'Get system logs (superuser only, last 15 entries)',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        method: 'POST',
        path: '/api/terminal/',
        desc: 'Execute simulated terminal command (hydra, nmap, whoami, status, help, clear)',
      },
    ],
  },
];
...
const sections = [
  ...
  { id: 'endpoints', label: 'All Endpoints', icon: <Globe className="h-4 w-4" /> },
  { id: 'examples', label: 'Code Examples', icon: <Code className="h-4 w-4" /> },
  { id: 'errors', label: 'Error Codes', icon: <FileText className="h-4 w-4" /> },
];
...
# }`,
  ...
  const errorCodes = [
    ...
    },
    {
      code: '403',
      name: 'Forbidden',
      description: 'Insufficient permissions (e.g., non-admin accessing admin endpoint)',
      severity: 'error',
    },
    {
      code: '404',
  ...
  const rateLimits = [
    ...
    {
      endpoint: 'Registration (/api/user/register/)',
      limit: 'Rate limited (RegisterRateThrottle)',
    },
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '64px 24px 48px' }}>
          ...
          >
            <div className="mb-4 flex items-center gap-3">
              <div className="rounded-lg p-2" style={{ background: accentDim }}>
              ...
              <button
                ...
                className="ml-auto flex items-center gap-2 rounded-lg px-3 py-1.5 font-mono text-xs transition-all"
                style={{
            ...
            <p style={{ fontSize: 17, color: C.muted, marginBottom: 32, lineHeight: 1.7 }}>
              Complete documentation for the PIIcasso Targeted Wordlist Generator API. Generate
              custom password wordlists from personal information using AI pattern matching.
            </p>
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                to="/login"
                className="flex items-center gap-2 rounded-lg border px-6 py-3 font-semibold transition-all"
                style={{
            ...
            <div style={{ position: 'sticky', top: 24 }}>
              ...
              >
                <div className="mb-4 flex items-center gap-2">
                  <Wifi className="h-3 w-3 animate-pulse text-green-400" />
                  <span
                    className="text-xs font-bold uppercase tracking-wider text-green-400"
                    style={S.mono}
                  >
                    Online
                  </span>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span style={{ color: accent, fontWeight: 600, ...S.mono, fontSize: 11 }}>
                      piicasso.com/api
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span style={{ color: C.muted }}>Auth</span>
                    <span style={{ color: C.green, fontWeight: 600, ...S.mono, fontSize: 11 }}>
                      JWT Bearer
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span style={{ color: C.muted }}>Endpoints</span>
                    <span style={{ color: C.purple, fontWeight: 600, ...S.mono, fontSize: 11 }}>
                      37+
                    </span>
                  </div>
                </div>
                <div
                  ...
                  style={{ color: C.dim, ...S.mono, borderColor: C.border }}
                >
              ...
              >
                <div>
                  ...
                  <p style={{ color: C.muted, lineHeight: 1.7 }}>
                    PIIcasso is a{' '}
                    <strong style={{ color: C.text }}>Targeted Wordlist Generator</strong> that
                    creates custom password wordlists from personal information. Submit PII data
                    (names, dates of birth, pet names, favorite numbers) and receive a tailored
                    wordlist generated by AI pattern matching combined with the RockYou corpus.
                  </p>
                </div>
                  ...
                  {[
                    {
                      ...
                      desc: 'Pattern matching engine combines submitted PII with RockYou corpus to generate realistic password candidates.',
                    },
                    {
                      icon: <Shield className="h-5 w-5" />,
                      title: 'Password Analysis',
                      desc: 'Strength scoring, entropy calculation, breach detection via HIBP k-anonymity, and PII-context vulnerability analysis.',
                    },
                    {
                      icon: <Database className="h-5 w-5" />,
                      title: 'Team Collaboration',
                      desc: 'Create teams, manage members with role-based access, and communicate via built-in team chat.',
                    },
                    {
                      icon: <Globe className="h-5 w-5" />,
                      title: 'Global Analytics',
                      desc: 'Interactive globe visualization showing geo-located user activity with real-time incremental updates.',
                    },
                  ].map((item) => (
                    <div
                      key={item.title}
                ...
                >
                  ...
                  <p className="mt-3 text-sm" style={{ color: C.muted }}>
                    All endpoints are relative to this base URL. Authentication uses JWT Bearer
                    tokens obtained via <code style={{ color: accent }}>/api/user/token/</code>.
                  </p>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  >
                    ...
                    <ul className="space-y-2 text-sm" style={{ color: C.muted }}>
                      ...
                      <li className="flex items-center gap-2">
                        <div className="h-1.5 w-1.5 rounded-full bg-green-400" /> /api/user/token/
                      </li>
                      <li className="flex items-center gap-2">
                        <div className="h-1.5 w-1.5 rounded-full bg-green-400" />{' '}
                      ...
                      <li className="flex items-center gap-2">
                        <div className="h-1.5 w-1.5 rounded-full bg-green-400" /> /api/health/
                      </li>
                    </ul>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <ul className="space-y-2 text-sm" style={{ color: C.muted }}>
                      ...
                      <li className="flex items-center gap-2">
                        <div className="h-1.5 w-1.5 rounded-full" style={{ background: accent }} />{' '}
                        /api/password/analyze/ — Analyze passwords
                      </li>
                      <li className="flex items-center gap-2">
                        <div className="h-1.5 w-1.5 rounded-full" style={{ background: accent }} />{' '}
            ...
            {activeSection === 'authentication' && (
              ...
              >
                <div>
                  ...
                  <p style={{ color: C.muted, lineHeight: 1.7 }}>
                    PIIcasso uses JWT (JSON Web Tokens) via{' '}
                    <code style={{ color: accent }}>djangorestframework-simplejwt</code>. All
                ...
                >
                  ...
                  <ol className="space-y-4">
                    {[
                      <span key="register">
                        Register via <code style={{ color: accent }}>POST /api/user/register/</code>{' '}
                        with username, email, password
                      </span>,
                      <span key="tokens">
                        Obtain tokens via{' '}
                        <code style={{ color: accent }}>POST /api/user/token/</code> — receive
                      ...
                      <span key="refresh">
                        Refresh expired tokens via{' '}
                        <code style={{ color: accent }}>POST /api/user/token/refresh/</code> with
                        refresh token
                      </span>,
                    ].map((step, i) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <Shield className="mt-0.5 h-5 w-5" style={{ color: C.amber }} />
                    <div>
                      <h4 style={{ fontWeight: 600, color: C.amber, marginBottom: 8 }}>
                        Security Notes
                      </h4>
                      <ul className="space-y-1.5 text-sm" style={{ color: 'rgba(245,158,11,0.7)' }}>
                        <li>
                          • Access tokens expire after a configurable duration (default: 5 minutes)
                        </li>
            ...
            {activeSection === 'endpoints' && (
              ...
              >
                <div>
                  ...
                  <p style={{ color: C.muted, lineHeight: 1.7 }}>
                    Complete endpoint reference organized by functional category. Click a category
                    to expand.
                  </p>
                </div>
                  ...
                  {endpointGroups.map((group) => (
                    ...
                    >
                      <button
                        ...
                        style={{
                          background: expandedGroups.includes(group.id)
                        ...
                        <div style={{ borderTop: `1px solid ${C.border}` }}>
                          {group.endpoints.map((ep) => (
                            ...
                            >
                              ...
                              <code
                                className="text-sm"
                                style={{ ...S.mono, color: C.text, minWidth: 220 }}
                              >
                                {ep.path}
                              </code>
                  ...
                  >
                    {rateLimits.map((rl, i) => (
                      <div
                        ...
                        style={{ borderBottom: `1px solid ${C.border}` }}
                      >
                        <code className="text-sm" style={{ ...S.mono, color: C.text }}>
                          {rl.endpoint}
                        </code>
                        <span className="font-mono text-sm" style={{ color: C.amber }}>
                          {rl.limit}
                        </span>
                      </div>
                    ))}
            ...
            {activeSection === 'examples' && (
              ...
              >
                <div>
                  ...
                  <p style={{ color: C.muted, lineHeight: 1.7 }}>
                    Common workflows using curl. Replace{' '}
                    <code style={{ color: accent }}>&lt;access_token&gt;</code> with your actual JWT
                    token.
                  </p>
                </div>
                ...
                {[
                  {
                    ...
                    desc: 'Submit personal information to create a targeted password wordlist.',
                    code: curlExamples.submit,
                    label: 'submit.sh',
                    icon: <Cpu className="h-5 w-5" />,
                    category: 'Generation',
                  },
                  {
                    ...
                    desc: 'Check a password against strength criteria and PII exposure.',
                    code: curlExamples.analyze,
                    label: 'analyze.sh',
                    icon: <Shield className="h-5 w-5" />,
                    category: 'Security',
                  },
                  {
                    ...
                    desc: 'Retrieve paginated history of all your wordlist generations.',
                    code: curlExamples.history,
                    label: 'history.sh',
                  ...
                  {
                    title: 'Create a Team',
                    desc: 'Set up a team with invite codes for collaborative operations.',
                    code: curlExamples.teamCreate,
                    label: 'team.sh',
                    icon: <Database className="h-5 w-5" />,
                    category: 'Teams',
                  },
                  {
                    title: 'Search Data Breaches',
                    desc: 'Check if an email or password has been exposed in known breaches.',
                    code: curlExamples.breachSearch,
                    label: 'breach.sh',
                  ...
                  >
                    <div style={{ padding: 24, borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <h3 style={{ fontSize: 17, fontWeight: 600, color: C.text, marginBottom: 4 }}>
                        {example.title}
                      </h3>
              ...
              >
                ...
                >
                  ...
                  <pre
                    ...
                    style={{ ...S.mono, color: C.muted, background: '#0a0a0e' }}
                  >
                    {`{
...
// Or for validation errors:
{
  ...
  "password": ["This password is too short."]
                ...
                >
                  ...
                  <ul className="space-y-2 text-sm" style={{ color: C.muted }}>
                    <li className="flex items-start gap-2">
                      <Eye className="mt-0.5 h-4 w-4 flex-shrink-0" style={{ color: C.amber }} />{' '}
                      <span>
                        <strong style={{ color: C.text }}>403</strong> — Token expired (60s TTL) or
                        invalid signature
                      </span>
                    </li>
                    <li className="flex items-start gap-2">
                      <Trash2 className="mt-0.5 h-4 w-4 flex-shrink-0" style={{ color: C.red }} />{' '}
                      <span>
                        <strong style={{ color: C.text }}>404</strong> — Requested file or
                        generation record not found
                      </span>
                    </li>
                    <li className="flex items-start gap-2">
                      <Lock className="mt-0.5 h-4 w-4 flex-shrink-0" style={{ color: C.blue }} />{' '}
                      <span>
                        <strong style={{ color: C.text }}>401</strong> — Unauthenticated access to
                        download endpoint
                      </span>
                    </li>
                  </ul>
      ...
      <div
        style={{
          background: `linear-gradient(to top, ${C.surface}, ${C.bg})`,
      ...
      >
        <div style={{ maxWidth: 700, margin: '0 auto', padding: '64px 24px', textAlign: 'center' }}>
          <h2 style={{ fontSize: 32, fontWeight: 700, marginBottom: 16 }}>
            Start Generating Wordlists
          </h2>
          <p className="mb-8" style={{ color: C.muted, lineHeight: 1.7 }}>
            Create your account to access the full API. Generate targeted wordlists, analyze
            password strength, and collaborate with your team.
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            ...
            <Link
              to="/login"
              className="flex items-center gap-2 rounded-lg border px-8 py-4 font-semibold transition-all"
  ...
  if (isAuthenticated) {
    return <DesignAppShell activeKey="learn">{pageContent}</DesignAppShell>;
  }
