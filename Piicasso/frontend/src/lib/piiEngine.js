...
export const PII_PATTERNS = [
  ...
  { type: 'SSN', label: 'ssn', re: /\b\d{3}-\d{2}-\d{4}\b/g, weight: 0.3 },
  { type: 'CARD', label: 'card', re: /\b(?:\d[ -]*?){13,16}\b/g, weight: 0.3 },
  {
    type: 'DOB',
    label: 'dob',
  ...
  { type: 'IP', label: 'ip', re: /\b\d{1,3}(?:\.\d{1,3}){3}\b/g, weight: 0.1 },
  {
    type: 'ADDR',
    label: 'address',
    re: /\b\d{1,5}\s+[A-Z][a-z]+\s(?:St|Street|Ave|Avenue|Rd|Road|Blvd|Lane|Ln|Drive|Dr)\b/g,
    weight: 0.14,
  },
  { type: 'ZIP', label: 'zip', re: /\b\d{5}(?:-\d{4})?\b/g, weight: 0.06 },
  {
    type: 'NAME',
    label: 'name',
    re: /\b(?:Mr|Ms|Mrs|Dr)\.?\s+[A-Z][a-z]+\s+[A-Z][a-z]+\b|\b[A-Z][a-z]{2,}\s[A-Z][a-z]{2,}\b/g,
    weight: 0.18,
  },
];
...
export function scorePassword(pw, profile = {}) {
  ...
  for (const c of commons) {
    if (lowPw.includes(c)) {
      penalty += 25;
      reasons.push({ kind: 'common', label: `Common token "${c}"` });
      break;
    }
  }
  if (/(.)\1{2,}/.test(pw)) {
    penalty += 8;
    reasons.push({ kind: 'pattern', label: 'Repeated characters' });
  }
  if (/0123|1234|2345|3456|4567|5678|6789|abcd|qwer|asdf/i.test(pw)) {
    penalty += 12;
    reasons.push({ kind: 'pattern', label: 'Sequential characters' });
  }
  if (/(19|20)\d{2}$/.test(pw)) {
    penalty += 14;
    reasons.push({ kind: 'pattern', label: 'Year suffix detected' });
  }
