...
const TargetForm = ({ onFormUpdate }) => {
  ...
  const theme = {
    ...
    accentGlow: isSecurityMode
      ? 'shadow-[0_0_15px_rgba(229,9,20,0.6)]'
      : 'shadow-[0_0_15px_rgba(37,99,235,0.6)]',
    inputBg: isSecurityMode
      ? 'bg-black/50 focus:bg-zinc-900/80 border-zinc-800 focus:border-security-red'
      : 'bg-white/5 focus:bg-white/10 border-white/10 focus:border-user-cobalt',
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-transparent">
      {/* Top Navigation Tabs */}
      <div
        className={`custom-scrollbar flex overflow-x-auto border-b ${isSecurityMode ? 'border-zinc-800' : 'border-white/10'} shrink-0 bg-black/40 backdrop-blur-md`}
      >
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={`min-w-[70px] flex-1 border-r p-2 md:p-3 ${isSecurityMode ? 'border-zinc-800' : 'border-white/10'} relative flex flex-col items-center justify-center gap-1 transition-all ${isSelected ? (isSecurityMode ? 'bg-zinc-900' : 'bg-white/10') : 'hover:bg-white/5'}`}
            >
              {isSelected && (
                <motion.div
                  layoutId="navTab"
                  className={`absolute left-0 top-0 h-0.5 w-full ${theme.accentBg} ${theme.accentGlow}`}
                />
              )}
              <cat.icon className={`h-4 w-4 ${isSelected ? theme.accentText : 'text-gray-500'}`} />
              <span
                className={`text-[9px] font-bold uppercase tracking-wider md:text-[10px] ${isSelected ? 'text-white' : 'text-gray-500'}`}
              >
                {cat.label}
              </span>

              {/* Tiny progress dot indicator */}
              <div className="mt-1 h-0.5 w-full max-w-[30px] overflow-hidden rounded-full bg-black/50">
                <div
                  className={`h-full ${completion > 0 ? theme.accentBg : 'bg-transparent'}`}
      ...
      {/* Input Canvas */}
      <div className="custom-scrollbar relative flex-1 overflow-y-auto bg-gradient-to-b from-black/20 to-black/60 p-4 md:p-5">
            ...
            >
              {FIELDS[selectedCategory].map((field) => (
                <div key={field.name} className="group relative">
                  <label
                    className={`mb-1.5 block flex items-center gap-1.5 font-mono text-[10px] font-bold uppercase tracking-widest ${isSecurityMode ? 'text-security-red/80' : 'text-user-cobalt/80'}`}
                  >
                    <Hash className="h-3 w-3 opacity-60" /> {field.label}
                  </label>
                  <div className="relative">
                    <input
                      ...
                      className={`w-full rounded-md border px-3 py-3 font-mono text-sm text-white placeholder-gray-700 outline-none transition-all md:py-2.5 md:text-xs ${theme.inputBg}`}
                      autoComplete="off"
                    />
                  </div>
                </div>
        ...
        {loading && (
          ...
          >
            <motion.div
              ...
              className={`mb-4 h-12 w-12 rounded-full border-2 border-t-transparent ${theme.accentBg}`}
            />
            <p className={`font-mono text-sm uppercase tracking-widest ${theme.accentText}`}>
              Analyzing PII Patterns
            </p>
            <p className="mt-1 font-mono text-xs text-gray-500">Gemini AI generating wordlist...</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* Footer / Submit Area */}
      <div
        className={`border-t p-3 md:p-4 ${isSecurityMode ? 'border-zinc-800 bg-black/80' : 'border-white/10 bg-black/40'} flex shrink-0 flex-col items-center justify-between gap-3 backdrop-blur-xl sm:flex-row`}
      >
        <div className="flex items-center gap-2">
          <Shield className={`h-4 w-4 ${isSecurityMode ? 'text-green-500' : 'text-user-cobalt'}`} />
        ...
        <button
          ...
          disabled={loading}
          className={`flex w-full items-center justify-center gap-2 rounded-md px-6 py-3 font-mono text-[10px] font-bold uppercase tracking-widest text-white transition-all sm:w-auto ${theme.accentBg} ${theme.btnHover} ${theme.accentGlow} ${loading ? 'cursor-not-allowed opacity-50' : ''}`}
        >
          {loading ? (
      ...
      {error && (
        <div className="absolute bottom-16 left-1/2 -translate-x-1/2 rounded border border-red-500 bg-red-900/90 px-4 py-1 text-xs text-white shadow-xl">
          {error}
        </div>
      )}
    </div>
