...
class TeamCreateTest(TestCase):
    ...
    def test_create_team(self):
        response = self.client.post('/api/teams/', {'name': 'Alpha Squad'})
    ...
    def test_create_team_already_in_team(self):
        self.client.post('/api/teams/', {'name': 'First Team'})
        response = self.client.post('/api/teams/', {'name': 'Second Team'})
...
class TeamJoinTest(TestCase):
    ...
    def test_join_by_invite_code(self):
        ...
        response = client.post('/api/teams/join/', {'invite_code': self.team.invite_code})
    ...
    def test_join_invalid_code(self):
        client = APIClient()
        client.force_authenticate(user=self.joiner)
        response = client.post('/api/teams/join/', {'invite_code': 'INVALID'})
...
class TeamChatTest(TestCase):
    ...
    def test_send_message(self):
        client = APIClient()
        client.force_authenticate(user=self.member)
        response = client.post('/api/teams/chat/', {'content': 'Hello team!'})
        self.assertEqual(response.status_code, 201)
    ...
    def test_non_member_cannot_chat(self):
        ...
        response = client.post('/api/teams/chat/', {'content': 'Intruder!'})
