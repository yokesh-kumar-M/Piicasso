...
def create_team(request):
    ...
    try:
        ...
        if TeamMembership.objects.filter(user=user).exists():
            return Response(
                {"error": "You are already a member of an active team."},
        ...
        if not name:
            return Response(
                {"error": "A unique team identification name is required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if len(name) > 100:
            return Response(
...
def join_team(request):
    ...
    try:
        ...
        if TeamMembership.objects.filter(user=user).exists():
            return Response(
                {"error": "Active deployment detected. Cannot join multiple teams."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not code:
            return Response(
                {"error": "Valid invitation credentials required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def team_chat_messages(request):
    ...
    try:
        ...
        return Response(
            {"error": "Unauthorized: Membership required for chat access."},
            status=status.HTTP_400_BAD_REQUEST,
        )
