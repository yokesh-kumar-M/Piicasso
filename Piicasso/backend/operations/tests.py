...
from operations.models import Message, Notification, SystemSetting
...
class NotificationTest(TestCase):
    ...
    def test_mark_notification_read(self):
        ...
        response = self.client.post("/api/operations/notifications/", {"id": notif.id}, format="json")
        self.assertEqual(response.status_code, 200)
        notif.refresh_from_db()
        self.assertTrue(notif.is_read)

    def test_mark_all_read(self):
        response = self.client.post("/api/operations/notifications/", {"action": "mark_all_read"}, format="json")
        self.assertEqual(response.status_code, 200)
...
class MessagingTest(TestCase):
    ...
    def test_user_sends_message_to_admin(self):
        ...
        response = client.post("/api/operations/messages/", {"content": "Help me!"}, format="json")
...
class BreachSearchTest(TestCase):
    def setUp(self):
        ...
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)
        self.get_throttles_patcher = patch("operations.views.BreachSearchView.get_throttles", return_value=[])
        self.get_throttles_patcher.start()
        self.addCleanup(self.get_throttles_patcher.stop)

    def test_breach_search_empty_query(self):
        response = self.client.post("/api/operations/breach-search/", {"query": ""}, format="json")
    ...
    @patch("requests.get")
    @patch("operations.views.k_anonymity_breach_count", return_value=0)
    def test_breach_search_email_uses_hibp_account_path(self, mock_breach_count, mock_get):
    ...
    def test_breach_search_password_check(self, mock_breach_count):
        ...
        response = self.client.post("/api/operations/breach-search/", {"query": "password123"}, format="json")
        self.assertEqual(response.status_code, 200)
        self.assertIn("password_exposures", response.data)
        self.assertEqual(response.data["password_exposures"], 42)

    @patch("operations.views.k_anonymity_breach_count", return_value=0)
    def test_breach_search_non_email_uses_internal_matches_safely(self, mock_breach_count):
        response = self.client.post("/api/operations/breach-search/", {"query": "password123"}, format="json")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["internal_matches"], 0)
    ...
    def test_unauthenticated_breach_search(self):
        client = APIClient()
        response = client.post("/api/operations/breach-search/", {"query": "test"}, format="json")
