    ...
    def post(self, request):
        ...
        try:
            ...
            if not wordlist:
                ...
                logger.info(
                    f"Background generation task queued for user={request.user.username} cache_key={cache_key}"
                )
                ...
                return Response(
                    {
                        ...
                        "cache_key": cache_key,
                        "status": "processing",
                        "websocket_group": f"gen_user_{user_id}"
                        if user_id
                        else f"gen_anon_{client_id}",
                    },
            ...
            logger.info(
                f"Generation retrieved from cache user={request.user.username} count={len(wordlist)}"
            )
            return Response(
                {"wordlist": wordlist, "id": record.id, "status": "cached"},
...
def export_history_csv(request):
    try:
        if request.user.is_superuser:
            ...
            rows = GenerationHistory.objects.filter(user=request.user).order_by(
                "-timestamp"
            )
...
def get_cached_wordlist(request, cache_key):
    ...
    if wordlist:
        ...
        if request.user.is_authenticated:
            record = (
                GenerationHistory.objects.filter(user=request.user, wordlist=wordlist)
                .order_by("-timestamp")
                .first()
            )
            if record:
        ...
        return Response({"wordlist": wordlist, "id": history_id, "status": "complete"})
    return Response(
