    ...
    def post(self, request):
        ...
        try:
            ...
            if cached_list:
                ...
                if len(wordlist_list) > max_size:
                    wordlist_list = wordlist_list[:max_size]

                # Cache the list
                cache.set(cache_key, wordlist_list, timeout=60 * 60 * 24)

            # Save to history (wordlist is a JSONField — stores list)
            ...
            create_notification(
                ...
                description=f"Generated {len(wordlist_list)} password candidates.",
                link="/dashboard",
            )

            logger.info(
                f"Generation complete user={request.user.username} count={len(wordlist_list)}"
            )
            return Response(
                {"wordlist": wordlist_list, "id": record.id, "status": "success"},
                status=status.HTTP_201_CREATED,
            )
