...
class PiiSubmitView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            import hashlib
            import json
            from .tasks import generate_wordlist_task
            
            # Create a deterministic hash of the PII data + pattern mode
            cache_key_data = json.dumps(pii_data, sort_keys=True) + pattern_mode
            cache_key = f"wordgen_{hashlib.md5(cache_key_data.encode()).hexdigest()}"
            
            wordlist = cache.get(cache_key)
            
            if not wordlist:
                client_id = request.query_params.get('client_id')
                user_id = request.user.id if request.user.is_authenticated else None
                
                # Triggers Async Celery Task for Wordlist Generation instead of blocking
                generate_wordlist_task.delay(
                    pii_data=pii_data,
                ...
                # Since the processing happens in the background, we don't save to the DB here
                # The Celery task will save the record and create the notification upon completion
                
                # Also log the async start
                UserActivity.objects.create(
                    activity_type='GENERATE',
                    description=f"Background generation started by {request.user.username}",
                    city="Secure Node",
                )
                
                logger.info(f"Background generation task queued for user={request.user.username} cache_key={cache_key}")
                ...
                return Response(
                    {
                        ...
                        "cache_key": cache_key, 
                        "status": "processing",
                        "websocket_group": f"gen_user_{user_id}" if user_id else f"gen_anon_{client_id}"
                    },
                    status=status.HTTP_202_ACCEPTED
                )

            # If cache hits, we can return immediately
            if not wordlist:
            ...
            UserActivity.objects.create(
                activity_type='GENERATE',
                description=f"Intelligence dossiers retrieved from cache by {request.user.username}",
                city="Secure Node",
            )

            logger.info(f"Generation retrieved from cache user={request.user.username} count={len(wordlist)}")
            return Response({"wordlist": wordlist, "id": record.id, "status": "cached"}, status=status.HTTP_200_OK)
...
class SimulatedTerminalView(APIView):
    ...
    def post(self, request):
        ...
        if cmd_base == 'hydra':
            ...
            if is_god:
                output.append(f"[22][ssh] host: 10.10.1.5   login: {target_user}   password: password123")
...
def get_cached_wordlist(request, cache_key):
    ...
    if wordlist:
        # Get history ID if authenticated user
        history_id = None
        if request.user.is_authenticated:
            record = GenerationHistory.objects.filter(user=request.user, wordlist=wordlist).order_by('-timestamp').first()
            if record:
