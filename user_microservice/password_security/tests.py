...
class PasswordSecurityTests(TestCase):
    ...
    def test_password_analyze_no_password(self):
        response = self.client.post('/api/user/password/analyze/', {}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_password_analyze_weak_password(self):
        response = self.client.post('/api/user/password/analyze/', {'password': 'password'}, format='json')
    ...
    def test_password_analyze_strong_password(self):
        response = self.client.post('/api/user/password/analyze/', {'password': 'CorrectHorseBatteryStaple!@#123'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue('score' in response.data)
    ...
    def test_user_preferences(self):
        # Update preferences
        response = self.client.put('/api/user/password/preferences/', {'default_mode': 'security', 'last_mode': 'security'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['default_mode'], 'security')

        # Get preferences
        response = self.client.get('/api/user/password/preferences/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['default_mode'], 'security')
